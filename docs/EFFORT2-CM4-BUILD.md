...
**RESUME HERE (fresh session on mcore1):** verify post-reboot — `/dev/spidev0.0`, `halow0`, and GPS readable on `/dev/serial0` **without sudo** (`gps_check.py`). Then wire `ingest/meshcore_monitor.py run()` to the `chan_monitor.py` pattern → positions + messages + rf into the DB; add `jtak-meshcore.service`; strip the dormant Meshtastic path.
